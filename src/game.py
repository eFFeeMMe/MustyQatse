...
from .physics.geometry import point_on_line
...
from .settings import MAX_BALL_BUMPS, FRICTION, GRAVITY
...
class Level:
    ...
    def load(self):
        ...
        self.blocks |= set(
            Block(Circle(480 - 192 + 384 / 16 * i, 250 + i % 2 * 24, 16.0))
            for i in range(16)
        )
        ...
        r1 = 48.0 + 16.0
        self.blocks |= set(
            Block(Arc(300.0, 128.0, r0, r1, i * pi / n, pi / n * (i + 0.9)))
            for i in range(n)
        )
        self.blocks |= set(
            Block(Arc(480.0, 80.0, r0, r1, i * pi / n, pi / n * (i + 0.9)))
            for i in range(n)
        )
        self.blocks |= set(
            Block(Arc(660.0, 128.0, r0, r1, i * pi / n, pi / n * (i + 0.9)))
            for i in range(n)
        )

        self.blocks |= set(
            Block(Capsule(32.0 + 48.0 * i, 360.0, 48.0 + 48.0 * i, 360.0, 8.0))
            for i in range(12)
        )
        self.blocks |= set(
            Block(Rectangle(32.0 + 48.0 * i, 400.0, 24.0, 16.0)) for i in range(13)
        )
...
class Ball(object):
    ...
    def update(self):
        ...
        # Verlet integration
        geom.x, geom.xp = geom.x * (2.0 - FRICTION) - geom.xp * (1.0 - FRICTION), geom.x
        geom.y, geom.yp = geom.y * (2.0 - FRICTION) - geom.yp * (1.0 - FRICTION), geom.y
...
class Catcher:
    ...
    def update(self):
        geom = self.geom
        geom.x = geom.x * (1.0 - self.fix) + pygame.mouse.get_pos()[0] * self.fix
...
class Game(EventHandler):
    ...
    def newTurn(self):
        ...
        for block in blocksToRemove:
            self.particleSystem.add(
                Explosion(block.rect.centerx, block.rect.centery, 18)
            )
    ...
    def addScore(self):
        self.score += 100.0 * self.multiplier
        self.multiplier += 0.5
        ...
        for ball in self.balls:
            ...
            for block in self.level.quadTree.hit(ball.rect):
                ...
                if dx is not None:
                    ...
                    # Bounce
                    px, py = point_on_line(
                        bg.xp, bg.yp, bg.x, bg.y, bg.x - dx, bg.y - dy
                    )
                    bg.xp = px * 2.0 - bg.xp
                    bg.yp = py * 2.0 - bg.yp
    ...
    def update(self):
        ...
        for ball in self.balls:
            if ball.geom.y > self.level.h + ball.geom.r:
                deadBalls.add(ball)
            elif ball.bumps > MAX_BALL_BUMPS:
                deadBalls.add(ball)
